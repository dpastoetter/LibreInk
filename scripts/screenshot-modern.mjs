/**
 * Takes screenshots of the modern app (index.html) home screen (light and dark) and saves to docs/screenshots/.
 * Requires: npm run build, then npm run screenshot:modern (starts preview, captures, exits).
 */
...
async function main() {
  ...
  try {
    ...
    await page.screenshot({
      path: path.join(screenshotsDir, 'light-mode.png'),
      type: 'png',
    });
    console.log('Saved docs/screenshots/light-mode.png');
    ...
    if (themeBtn) {
      ...
      console.log('Saved docs/screenshots/dark-mode.png');
    }
  } finally {
    await browser.close();
    killPreview();
