...
- **Single-file legacy (preferred):** The build produces a single IIFE bundle (`openink-legacy-single.js`) with no SystemJS or dynamic imports. When present, `legacy.html` loads only that one script for maximum compatibility on very old WebKit (e.g. Kindle). If the single build was not run, the generator falls back to polyfill + `System.import(entry)`.
- **Mounted flag:** After the app renders, it sets `window.__openinkMounted = true`. The 12s fallback timer clears when the app mounts (single script runs synchronously; polyfill path clears when `System.import(entry)` resolves).
