...
const LAZY_APPS: { descriptor: AppDescriptor; load: () => Promise<WebOSApp> }[] = [
  ...
  { descriptor: { id: 'readlater', name: 'Read later', icon: '🔖', iconFallback: '+', iconLegacySvg: LEGACY_ICONS.readlater, category: 'reader' }, load: () => import('./readlater').then((m) => m.readlaterApp) },
